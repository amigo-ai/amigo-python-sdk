...
class AsyncRoleResource:
    ...
    async def get_roles(self) -> RoleGetRolesResponse:
        response = await self._http.request("GET", f"/v1/{self._organization_id}/role/")

        return RoleGetRolesResponse.model_validate_json(response.text)

    async def create_role(self, body: RoleCreateRoleRequest) -> RoleCreateRoleResponse:
...
class RoleResource:
    ...
    def get_roles(self) -> RoleGetRolesResponse:
        response = self._http.request("GET", f"/v1/{self._organization_id}/role/")

        return RoleGetRolesResponse.model_validate_json(response.text)

    def create_role(self, body: RoleCreateRoleRequest) -> RoleCreateRoleResponse:
